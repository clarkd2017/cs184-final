...
/**
 * Initializes the water simulation and spawns a new thread to separate
...
void WaterSimulator::init() {
  ...
  for (auto &pm : water->point_masses) {
    avg_pm_position += pm.position / water->point_masses.size();
  }

  CGL::Vector3D target(avg_pm_position.x, avg_pm_position.y / 2,
                       avg_pm_position.z);
  CGL::Vector3D c_dir(0., 0., 0.);
  canonical_view_distance = max(water->width, water->height) * 0.9;
